...
8. **BUDGET + COMMIT DISCIPLINE** — wall-clock budget, then verbatim: *"Commit PER COMPLETED ITEM. If you near the budget: COMMIT everything verified, write an honest partial report naming exactly what is un-run, and end — never die silent, never background-and-await."* If the builder may push, name the exact leaf branch: pushes go to **the feature branch named in the header only — never to a shared branch** (`main`/`dev`/`sprint`); promotion up the hierarchy is always a separate human-blessed ship dispatch.
