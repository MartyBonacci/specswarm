...
4. Enhanced: SpecTest plugin with hooks and metrics
   by Marty Bonacci & Claude Code (2025)
-->
...
<!-- ========== PRE-PLAN HOOK (SpecTest Enhancement) ========== -->

## 🎣 Pre-Plan Hook

**Purpose**: Validate prerequisites and preview tech stack status

1. **Display Hook Banner**: `🎣 Pre-Plan Hook`

2. **Spec Validation**:
   - Verify spec.md exists and is complete
   - Check for [NEEDS CLARIFICATION] markers (should be 0)

3. **Tech Stack Preview**:
   - If tech-stack.md exists: Display current stack summary
   - If first feature: Preview auto-detection process

4. **Metrics Initialization**: Record plan start time

5. **Display Readiness**: Show prerequisites status

6. **Proceed to outline**
...
<!-- ========== POST-PLAN HOOK (SpecTest Enhancement) ========== -->

## 🎣 Post-Plan Hook

**Purpose**: Validate plan quality and save metrics

1. **Display Hook Banner**: `🎣 Post-Plan Hook`

2. **Plan Consistency Check**:
   - Verify plan matches spec requirements
   - Check Technical Context completeness

3. **Tech Stack Summary**:
   - Display auto-added libraries (if any)
   - Display resolved conflicts (if any)
   - Confirm no prohibited technologies

4. **Metrics Recording**:
   - Save plan duration, auto-adds, conflicts to `/memory/metrics.json`

5. **Contract Generation Suggestion**: If entities exist, suggest generating API contracts

6. **Next Step**: Suggest `/spectest:tasks`
