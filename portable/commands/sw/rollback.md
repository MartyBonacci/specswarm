...
if [ "$ROLLBACK_STRATEGY" = "Soft rollback (revert commits)" ] && [ "$SWITCH_BRANCH" = false ]; then
  ...
  echo "   3. Merge to parent: /sw:complete"
  echo ""
elif [ "$SWITCH_BRANCH" = true ]; then
  echo "💡 Next Steps:"
  echo "   1. Review current state: git status"
  echo "   2. Start a new feature: /sw:build \"feature\""
  echo ""
fi
...
- Consider using `/sw:deprecate` for features in production
