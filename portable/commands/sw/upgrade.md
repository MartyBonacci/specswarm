...
if [ -z "$UPGRADE_TARGET" ] && [ "$UPGRADE_ALL_DEPS" = false ] && [ -z "$PACKAGE_NAME" ]; then
  ...
  echo ""
  echo "Usage: /sw:upgrade \"target\" [--deps] [--package name] [--dry-run]"
  echo ""
  echo "Examples:"
  echo "  /sw:upgrade \"React 18 to React 19\""
  echo "  /sw:upgrade \"Next.js 14 to Next.js 15\""
  echo "  /sw:upgrade --deps                    # All dependencies"
  echo "  /sw:upgrade --package react           # Specific package"
  echo "  /sw:upgrade \"Vue 2 to Vue 3\" --dry-run"
  exit 1
fi
...
if [ "$DRY_RUN" = true ]; then
  ...
  echo "   /sw:upgrade \"$UPGRADE_TARGET\""
  echo ""
