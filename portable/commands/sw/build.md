...
```bash
echo "SpecSwarm Build - Complete Feature Development"
echo "=============================================="
echo ""
...
```
Use the SlashCommand tool to execute: /sw:specify "$FEATURE_DESC"
```
...
echo "----------------------------------------"
echo ""
echo "INTERACTIVE: Please answer the clarification questions."
echo ""
```

```
Use the SlashCommand tool to execute: /sw:clarify
```
...
```bash
echo "----------------------------------------"
echo "Phase 3: Generating Implementation Plan"
echo "----------------------------------------"
echo ""
```

```
Use the SlashCommand tool to execute: /sw:plan
```
...
```bash
echo "----------------------------------------"
echo "Phase 5: Implementing Feature"
echo "----------------------------------------"
echo ""
...
```
Use the SlashCommand tool to execute: /sw:validate
```
...
if [ "$QUALITY_SCORE" -lt "$QUALITY_GATE" ]; then
  echo "WARNING: Quality score (${QUALITY_SCORE}%) below threshold (${QUALITY_GATE}%)"
...
**Before** (Manual):
```bash
/sw:specify "feature description"
/sw:clarify
/sw:plan
/sw:tasks
/sw:implement
...
**After** (Build):
```bash
/sw:build "feature description" --validate
